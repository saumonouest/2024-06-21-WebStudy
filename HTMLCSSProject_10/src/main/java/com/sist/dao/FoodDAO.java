 ...
 *
 *	2. SQL 문장 전송 / 수신
 *	   PreparedStatement : BufferedReader / OutputStream - executeQuery, executeUpdate
 *							ㄴ ResultSet => 브라우저 전송 : List / VO	
 *	====================================================================================
 *				요청 => 요청값 받기(request)
 *	브라우저 <==============> 서버(DAO, JSP)	  
 *			  응답(HTML)
...
public class FoodDAO {
	 ...
	 *	UPDATE : 게시판 수정, 댓글 수정, 회원가입 수정, 장바구니 수정, 예약 수정
	 *	DELETE : 게시판 삭제, 댓글 삭제, 회원탈퇴, 장바구니 취소, 예약 취소
	 *	SELECT : 나머지
	 */
	public List<FoodVO> foodListData(int type, int page){
		...
		try {
			...
			/*
					...
					+"WHERE num BETWEEN ? AND ?";
			 */
			// rownum은 Top-N => 처음부터는 가능한데, 중간은 자르지 못 함 => 지금처럼 인라인뷰 써서 바꿔놔야 함
			ps=conn.prepareStatement(sql);
			int rowSize=12;
			int start=(rowSize*page)-(rowSize-1);
			int end=rowSize*page;
			...
			ps.setInt(2, start);
			ps.setInt(3, end);
			// => 전체 / 베스트 / 특가 / 신상품 => 각각 테이블이 만들어져 있음
			ResultSet rs=ps.executeQuery();
			while(rs.next()) {
				FoodVO vo = new FoodVO();
				vo.setFno(rs.getInt(1));
				vo.setName(rs.getString(2));
				vo.setPhone(rs.getString(3).replace("https","http"));
				list.add(vo);
			}
			rs.close();
