...
public class BoardDAO {
	...
	public List<BoardVO> boardListData(int page){
		...
		try {
			...
			while(rs.next()) { // rs.next()는 데이터 출력의 첫번째 위치에 cursor를 이동
	...
	public BoardVO boardDetailData(int no) {
		// 상세보기 => 반드시 Primary Key 값을 넘겨준다 (중복이 없는 데이터)
		BoardVO vo = new BoardVO();
		try {
			conn=dbConn.getConnection();
	...
	public BoardVO boardUpdateData(int no) {
		...
		try {
			conn=dbConn.getConnection();
			String sql="SELECT no, name, subject, content, hit "
					+"FROM board "
					+"WHERE no="+no;
			...
			System.out.println("=== 업데이트 데이터 에러 ===");
			ex.printStackTrace();
		}
		finally {
			dbConn.disConnection(conn, ps);
