   ...
   <div class="row resRow">
    <table class="table">
     ...
     <c:forEach var="vo" items="${recvList }">
       <tr>
          <td class="text-center">${vo.frno }</td>
	      <td class="text-center">
	       <img src="https://www.bluer.co.ke${vo.fvo.poster }" style="width: 20px;height: 20px">
	      </td>
	      <td>${vo.fvo.name }</td>
	      ...
	      <td class="text-center inline">
	       <c:if test="${vo.isok=='y' }">
	        <span class="btn btn-success btn-xs infos" data-frno="${vo.frno }">예약완료</span>
	       </c:if>
	       <c:if test="${vo.isok=='n' }">
	        <span class="btn btn-default btn-xs">예약대기</span>
	       </c:if>
	        <a href="../mypage/mypage_food_reserve_cancel.do?frno=${vo.frno }" class="btn btn-danger btn-xs">취소</a>
	      </td>
       </tr>
     </c:forEach>
    </table>
