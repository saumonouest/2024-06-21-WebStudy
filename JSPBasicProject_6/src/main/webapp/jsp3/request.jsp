	...
	=====================================================
	 ...
	 = ASP, PHP
		...
		1. 서버 정보 / 브라우저 정보
			...
			================ 				================================== URI
			  서버IP / Port => 80은 생략이 가능 	================== ContextPath
			  => getServerName() => localhost				   => 구분자가 없다 : 파일명 자체가 구분자다
			  	 AWS : 각자마다 고정 IP를 부여
			  => getProtocol() : 약속 => http
			  => getPort() : 80
						  					=> getRequestURL()
...
<!-- 
	사용자가 전송하는 모든 데이터는 서버를 거쳐서 톰캣에 의해 request에 전송된 데이터가 묶여서 넘겨짐 
											  ===============================
											   	=> 처리를 웹 개발자가 한다
	묶는 방법 => Map 방식
			  ======== 키, 값 => getParameter("키")
			  ======== list.jsp?1&2&num3&4&5
	setAttribute(키, 값); 키는 중복이 불가능하다 ...
	
	=> getRequestURI()
		=> /프로젝트명/폴더명/파일명?id=admin&pwd=1234
		   ================== ================= request 첨부
		 			URI
-->
</head>
